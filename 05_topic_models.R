...
many_models <- readRDS("outputs/many_models")
dfm_stm <- readRDS("outputs/dfm_stm")
...
k_result %>% 
  ...
  labs(x = "Semantic coherence (mean)",
       y = "Exclusivity (mean)",
       title = "Comparison of models' semantic coherence and exclusivity") +
  theme_minimal() +
...
transl <- readLines("fig_7_translation.csv") %>% 
  str_remove_all('"')

gamma_terms %>%
  top_n(20, gamma) %>%
  ggplot(aes(topic, gamma, label = transl, fill = topic)) +
