...
df_veda_clean %>%
  ...
  arrange(Var1, desc(prop)) %>%
  # reorder the position of Var1 by the relative frequency (rel) and plot
  ggplot(aes(reorder(Var1, prop), weight=prop)) + labs(x="Slovo", y="Proporce korpusu", title = "Přírodní vědy") + coord_flip() + geom_bar() + theme_bw()
